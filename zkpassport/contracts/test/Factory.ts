...
describe("Factory Contract", async function () {
  ...
  it("Should deploy Auction contract through factory", async function () {
    ...
    const auction = await viem.getContractAt("Auction", auctionAddress as `0x${string}`);
    
    assert.equal(await auction.read.verifier().then(addr => (addr as `0x${string}`).toLowerCase()), verifier.address.toLowerCase());
  ...
  it("Should emit Deployed event when deploying contract", async function () {
    ...
    assert.equal((events[0].args as { contractAddress: `0x${string}` }).contractAddress.toLowerCase(), (auctionAddress as `0x${string}`).toLowerCase());
    assert.equal((events[0].args as { votingType: number }).votingType, 0); // CeremonyType.Auction
  });
  ...
  it("Should prevent deploying with same salt twice", async function () {
    ...
    try {
      ...
      assert.ok((error as Error).message.includes("SaltAlreadyUsed") || (error as Error).message.includes("revert"));
    }
  });
