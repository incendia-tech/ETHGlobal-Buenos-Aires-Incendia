...
describe("Auction Contract", async function () {
  ...
  it("Should accept valid proof and submit bid", async function () {
    ...
    // Wait for transaction and get the deployed address
    const receipt = await publicClient.waitForTransactionReceipt({ hash: txHash });
    const auctionAddress = await factory.read.contracts([salt]) as `0x${string}`;
    ...
    assert.equal((events[0].args as { bidder: `0x${string}` }).bidder.toLowerCase(), (await viem.getWalletClients().then(clients => clients[0].account.address)).toLowerCase());
  });
